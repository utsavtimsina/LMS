...
if ($action == 'e') {
    ...
    if (isset($_POST['updatebd'])) {
        ...
        $update_sql = "UPDATE `book_details_tbl` SET `bid`='$bid',`bname`='$bname',`book_id`='$book_id',`faculty_id`='$faculty_id',`subcode`='$subcode',`year`='$year',`semester`='$semester',`edition`='$edition',`category_id`='$category_id',`publication`='$publication',`author`='$author',`price`='$price' WHERE bid=$bid";


        $result = mysqli_query($conn, $update_sql) or die('ddgsh');
        if ($result) {
