<?php 
    ...
    if(isset($_POST['change']) && $a==2){
        ...
        if($new_password===$re_password){
            $sql= "SELECT * FROM `lib_tbl` WHERE `empid`='$_SESSION[lib_uid]'";
            $result = mysqli_query($conn,$sql);
            if(mysqli_num_rows($result)>0){
                $row=mysqli_fetch_assoc($result);

                if($old_password===$row['password']){
                    $change_sql="UPDATE `lib_tbl` SET `password`='$new_password' WHERE `empid`='$_SESSION[lib_uid]'";
        ...
        header("location:adminChangePassword.php?a=1&msg=$msg");
    }
?>
    ...
    <div class="content">
        ...
        ?>
            <div class="msg" id="impmsg">
                <i class="fa-solid fa-plus cross" onclick="location.href='adminChangePassword.php?a=2'"></i>
                <?php
                @$msg = $_GET['msg'];
                echo $msg;
                ?>
